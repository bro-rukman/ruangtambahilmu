    ...
    return (
        <div>
            <div className="shadow-xl bg-black hover:bg-gray-600 rounded relative cursor p-3" style={{height:150}}>
                ...
                <p className="text-sm text-white">{item?.description ?? "description"}</p>
                <h4 className=" text-xl font-bold font-gold">Rp {formatNumber(item?.harga ?? "harga")}</h4>
                <Link href="/detail/[slug]" as={`detail/${item.slug}`}><a className="link-wrapper"></a></Link>
            </div>
        </div>
    )
}
